...
interface IShopPageMenuProps {
  ...
  onSelectedCategoryChanged: (value: string) => void;
...
export const ShopPageMenu = (props: IShopPageMenuProps) => (
  <React.Fragment>
    <Responsive maxWidth={650}>{renderTinyMenu(props)}</Responsive>
    <Responsive minWidth={650}>{renderNormalMenu(props)}</Responsive>
  </React.Fragment>
);

function renderNormalMenu(props: IShopPageMenuProps) {
  return (
    <Menu pointing fixed={props.fixToBottom ? "bottom" : undefined}>
      <Container>
        {props.orderOptions.map(o => (
          <Menu.Item
            ...
            onClick={(e, o) => props.onSelectedCategoryChanged(o.value + "")}
          />
        ))}
        <Menu.Menu position="right">
          <Menu.Item>
            <Input
              ...
              onChange={(e, { value }) => props.onSearchTermChanged(value)}
            />
          </Menu.Item>
        </Menu.Menu>
      </Container>
    ...
    <Menu pointing fixed={props.fixToBottom ? "bottom" : undefined}>
      <Container>
        <Dropdown
          ...
          onChange={(e, o) => props.onSelectedCategoryChanged(o.value + "")}
