  ...
  csvtojson()
    ...
    .on("done", async (error: any) => {
      try {
        ...
        for (var product of products) await generateThumbnail(product);
...
async function generateThumbnail(product: Partial<IPrint>): Promise<void> {
  ...
  const thumbPath = `${__dirname}/../images/products/thumb/${fname}`;
  ...
  product.thumb = `/images/products/thumb/${fname}`;
}
