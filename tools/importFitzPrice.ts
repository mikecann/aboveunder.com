...
interface ICSVRow {
  ...
  Width: string;
  Height: string;
  "Fitz Price (Old)": string;
  "Fitz Price (2020)": string;
  "My Price": string;
  Ratio: string;
  Area: string;
  Weight: string;
...
async function run() {
  ...
  const arr: ICSVRow[] = await csvtojson().fromFile(csvPath);
  ...
  for (let json of arr) {
    ...
    if (mapping == null) throw Error("No mapping found for: " + json.Product);
  ...
  var outp = [];
  for (var key in options) outp.push(options[key]);
