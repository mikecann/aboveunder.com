...
export async function askQuestion(question: string, defaultVal?: string): Promise<string> {
  ...
  return new Promise<string>((resolve, reject) => {
    rl.question(q + " ", answer => {
      if (!answer && defaultVal) answer = defaultVal;
...
export async function resizeImg(
  ...
  console.log(
    `Resizing image '${srcPath}' to '${destPath}' with width ${width} and quality ${quality}`
  );
...
export function extractLatLngFromExif(exif: ExifData): google.maps.LatLngLiteral {
  if (!exif || !exif.gps || !exif.gps.GPSLatitude) return undefined;
...
export function extractDateCreatedFromExif(exif: ExifData): string {
  if (!exif || !exif.exif || !exif.exif.CreateDate) return undefined;
